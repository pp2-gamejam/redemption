...
zombies_in_wave = 10
enemies_killed = 0
spawn_time_ranges = [(5000, 3000), (3000, 2000)]

def get_spawn_time():
    min_time, max_time = spawn_time_ranges[current_wave - 1]
...
while running:
    ...
    if enemies_killed < zombies_in_wave and current_time - last_spawn_time > get_spawn_time():
        spawn_point = random.choice([(600, 400), (750, 500), (700, 300)])  # Example spawn points
        enemy = Enemy(spawn_point[0], spawn_point[1], "zomzon1.png", target_sprites, barriers_group)  # Adjust enemy attributes per wave
        all_sprites_group.add(enemy)
    ...
    draw_text(screen, f"Wave: {current_wave}/{total_waves}", (10, 10))
    draw_text(screen, f"Killed: {enemies_killed}/{zombies_in_wave}", (10, 40))
    ...
    if enemies_killed >= zombies_in_wave:
        if current_wave < total_waves:
            current_wave += 1
            zombies_in_wave = 15 if current_wave == 2 else 10
            enemies_killed = 0
        else:
            running = False  # or implement an end game scenario
pygame.quit()
