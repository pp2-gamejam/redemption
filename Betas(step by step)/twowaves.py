...
#waves dictionary
waves = [
    {"count": 10, "health": 100, "spawn_time_range": (5000, 3000), "image": "zomzon1.1.png"},
    {"count": 15, "health": 200, "spawn_time_range": (6000, 3000), "image": "bigzomzon1.1.png"}
]
...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, x, y, health, image_path, target_sprites, barriers):
        super().__init__()
        ...
        self.max_health = health  # Max health for full health bar
...
enemy_spawn_y = 400
health_v=100
enemy = Enemy(enemy_spawn_x, enemy_spawn_y,health_v, enemy_image_path, target_sprites, barriers_group)
...
while running:
    ...
    if current_wave < len(waves):
        ...
        if (zombies_spawned_this_wave < wave['count'] and
            current_time - time_since_last_spawn > random.randint(*wave['spawn_time_range'])):
            spawn_point = random.choice(spawn_points)
            new_zombie = Enemy(spawn_point[0], spawn_point[1], wave['health'], wave['image'], target_sprites, barriers_group)
    ...
    # Check if it's time to move to the next wave
    if zombies_spawned_this_wave == wave['count'] and not any(isinstance(x, Enemy) for x in all_sprites_group):
        if current_wave + 1 < len(waves):
            current_wave += 1
            zombies_spawned_this_wave = 0
            print(f"Wave {current_wave + 1} starting.")
