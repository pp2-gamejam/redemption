            ...
            # Check for collisions with enemy
                for enemy in enemy_sprites:  
                    if self.attack_rect.colliderect(enemy.rect):
                        enemy.take_damage(1)  # Apply damage to each enemy
        if self.direction == "left":
...
time_between_spawns = 5000  # 5 seconds in milliseconds
...
while running:
    ...
    if (current_time - last_spawn_time > time_between_spawns and zombies_spawned_this_wave < zombies_in_wave):
        spawn_point = random.choice(spawn_points)
        new_zombie = Enemy(spawn_point[0], spawn_point[1], "zomzon1.1.png", target_sprites, barriers_group)
