...
pygame.mixer.init()
fail_sound = pygame.mixer.Sound("fail2.mp3")
win_sound = pygame.mixer.Sound("win1.wav")
...
knopki1_image = pygame.transform.scale(pygame.image.load("knopki1.1.png").convert_alpha(), (200, 100))
knopki1_pressed_image = pygame.transform.scale(pygame.image.load("knopki1.0.png").convert_alpha(), (200, 100))
knopki2_image = pygame.transform.scale(pygame.image.load("knopki2.2.png").convert_alpha(), (200, 100))
knopki2_pressed_image = pygame.transform.scale(pygame.image.load("knopki2.0.png").convert_alpha(), (200, 100))
game_over_image = pygame.transform.scale(pygame.image.load("gameover.png").convert(), (WIDTH, HEIGHT))
victory_image = pygame.transform.scale(pygame.image.load("win.png").convert(), (WIDTH, HEIGHT))
...
# Функция для отображения заставки с кнопками
def show_start_screen():
    screen.blit(zastavka_image, (0, 0))
    start_button_rect = pygame.Rect(WIDTH // 2 - 100, HEIGHT // 3, 200, 100)
    exit_button_rect = pygame.Rect(WIDTH // 2 - 100, HEIGHT // 2, 200, 100)
    start_button_image = knopki1_image
    exit_button_image = knopki2_image
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if start_button_rect.collidepoint(mouse_pos):
                    start_button_image = knopki1_pressed_image
                elif exit_button_rect.collidepoint(mouse_pos):
                    exit_button_image = knopki2_pressed_image
        ...
        screen.blit(zastavka_image, (0, 0))
        screen.blit(start_button_image, (WIDTH // 2 - 100, HEIGHT // 3))
        screen.blit(exit_button_image, (WIDTH // 2 - 100, HEIGHT // 2))
...
def show_game_screen():
    # Определение классов и остальной код игры
    ...
    class Player(pygame.sprite.Sprite):
        def __init__(self):
            ...
            self.bonk_sound = pygame.mixer.Sound("bonk_sound.mp3")
        ...
        def update_attack_rect(self):
            offset = 0
            size = (50, 40)  # Size of the attack rectangle
    ...
    class Enemy(pygame.sprite.Sprite):
        def __init__(self, x, y, image_path, target_sprites, barriers):
            super().__init__()
            self.kick_sound = pygame.mixer.Sound("kick.mp3")
            self.image = pygame.image.load(image_path).convert_alpha()
            self.image = pygame.transform.rotozoom(self.image, 0, 3) #чтобы увеличить
            ...
            self.knockback_distance = 10  # Distance to move the enemy back when hit
            self.is_alive = True  # Death flag
        ...
        def take_damage(self, damage):
            if not self.is_alive:
                return  # Ignore damage if the enemy is already dead
            print(f"Enemy taking {damage} damage.")
            self.health -= damage
            print(f"Enemy remaining health: {self.health}")
            self.health -= damage
            if self.health <= 0 and self.is_alive:
                self.is_alive = False  # Mark as dead to prevent multiple counts
                self.kill()  # Remove the enemy sprite from all groups
                global zombies_killed
                zombies_killed += 1  # Increment the global counter for killed enemies
                self.drop_item()
            else:
                self.knockback()
        ...
        def drop_item(self):
            global zombie_kill_count
            zombie_kill_count += 1
            if zombie_kill_count >= 5:
                item = Item(self.rect.centerx, self.rect.centery, "papka.png")
    ...
    class Item(pygame.sprite.Sprite):
        ...
        def apply(self, target):
            target.health += 20
            target.health = min(target.max_health, target.health)  # Don't exceed max health
    ...
    spawn_points = [(600, 400), (750, 500), (700, 300)]
    # Wave Management
    current_wave = 1
    total_waves = 5  # Total number of waves
    zombies_in_wave = 10 * current_wave
    zombies_spawned_this_wave = 0
    zombies_remaining = zombies_in_wave
    ...
    while running:
        ...
        if zombies_spawned_this_wave < zombies_in_wave and current_time - last_spawn_time > time_between_spawns:
            spawn_point = random.choice(spawn_points)
            new_zombie = Enemy(spawn_point[0], spawn_point[1], "zomzon1.1.png", [player], barriers_group)
            all_sprites_group.add(new_zombie)
            zombies_spawned_this_wave += 1
            zombies_remaining -= 1  
            last_spawn_time = current_time

        if zombies_remaining == 0 and current_wave < total_waves:
            current_wave += 1
            zombies_in_wave = 10 * current_wave
            zombies_spawned_this_wave = 0
            zombies_remaining = zombies_in_wave
            time_between_spawns = max(1000, 5000 - 800 * current_wave)  # Decrease spawn time with each wave

        # Display Wave Info
        screen.blit(background_image, (0, 0))
        draw_text(f"Wave: {current_wave}/{total_waves}", font, WHITE, screen, 5, 5)
        draw_text(f"Enemies Killed: {zombies_killed}", font, WHITE, screen, 5, 40)
        ...
        enemy_sprites = [enemy for enemy in all_sprites_group if isinstance(enemy, Enemy)]
        ...
        for sprite in all_sprites_group:
            if isinstance(sprite, Item) and not sprite.alive():
                all_sprites_group.remove(sprite)
        global fail_sound_played
        if (player.health <= 0 or Pc.health <= 0) and not fail_sound_played:
            game_over = True
            fail_sound.play()  # Play the fail sound
            fail_sound_played = True  # Set the flag to True so the sound is played only once
        global win_sound_played
        if current_wave >= total_waves and not any(isinstance(x, Enemy) for x in all_sprites_group):
            victory = True
            win_sound.play()
...
zombie_kill_count = 0
zombies_killed = 0
show_start_screen()  # Запуск заставки
show_game_screen()  # Запуск игры
pygame.quit()